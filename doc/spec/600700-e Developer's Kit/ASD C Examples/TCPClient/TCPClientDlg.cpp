	...
	: CDialog(CTCPClientDlg::IDD, pParent)
{
	...
	m_iVnirDarkCurrentCorrection = 0;
	m_yMin = 0;
	m_yMax = 65535;
	m_sYCaption = "rawDN";
...
void CTCPClientDlg::OnConnect() 
{
	...
	while( totalRecv < 50 ) 
	{
		...
		totalRecv +=bytesRecv;
        strcat_s(totalbuf, sizeof(totalbuf), recvbuf);
	...
	unsigned char vsbuf[sizeof(VersionStruct)] = "";
	int totalBytesRecv = 0;
	
    bytesSent = send( Socket, strCommand, strCommand.GetLength(), 0 );
   
	WaitForIOEvent(sizeof(*vs), vsbuf, &totalBytesRecv);

	unsigned char *p = &vsbuf[0];
	vs = (VersionStruct *)malloc(sizeof(*vs));
    ...
    vs->type = ntohl(vs->type);
	vs->value = swapWord(vs->value);

	// if 1.5 or greater then issue a RETORE,1 command to build the calibration arrays.
    if(vs->Header == 100)
	{
	    if(vs->value >= 1.5)
		{
			// Restore the Flash and build calibration arrays
			strCommand = "RESTORE,1";
			unsigned char rBuf[7616] = "";

			bytesSent = send( Socket, strCommand, strCommand.GetLength(), 0);

			WaitForIOEvent(7616, rBuf, &totalBytesRecv);

			HeaderStruct *hs = (HeaderStruct *)malloc(sizeof(*hs));
			p = &rBuf[0];

			memcpy(hs, p, sizeof(*hs));

			hs->Header = ntohl(hs->Header);
			hs->errbyte = ntohl(hs->errbyte);
			...
			{
				AfxMessageBox("Unable to Restore Flash", MB_OK | MB_ICONEXCLAMATION);
			}
			free(hs);
...
void CTCPClientDlg::OnVersion() 
{
	...
	s.Format("Instrument Type->%d", vs->type);
	m_Header.AddString(s);
...
void CTCPClientDlg::OnDarkCorrectedSpectrum() 
{
	...
	if(m_iVnirEndingWavelength == 0)
	{
		...
		GetFlashValue(ps, "INIT,0,VDarkCurrentCorrection");
		m_iVnirDarkCurrentCorrection = (int)swapWord(ps->value);
	...
	//
	MessageBox("Please cap the fiber to capture a Dark Current.", "Dark Current", MB_OK | MB_ICONEXCLAMATION);
	//
	// Close Shutter
	// Uncomment to following lines to close the shutter.
	// Note: This line will have no effect if there is no shutter in the instrument.
	// 
	// Close shutter with Acquire command
	...
	int dark_drift = ntohl(issDarkSpectrum->FRHeader.v_header.drift);
	...
	// necessary
	//
	MessageBox("Dark Current Complete.  Please uncap fiber.", "Dark Current", MB_OK | MB_ICONEXCLAMATION);
	//
	// Open Shutter
	...
	//
	// Open Shutter with Acquire command
	//Acquire(iss, "A,5,0");
	//
	// Alternative Shutter Command
	...
	iss->FRHeader.Header = ntohl(iss->FRHeader.Header);
	iss->FRHeader.errbyte = ntohl(iss->FRHeader.errbyte);
	int current_drift = ntohl(iss->FRHeader.v_header.drift);
	...
	free(os);
	free(iss);
	// Uncomment the following line if declared above for shutter control
	//free(ics);
