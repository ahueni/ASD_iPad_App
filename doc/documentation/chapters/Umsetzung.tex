...
In der Klasse \verb=SpectrometerViewController= sind alle Funktionen und VerknŸpfungen fŸr die Hauptansicht umgesetzt. Diese ist in der Lage, wenn alle Voraussetzungen ErfŸllt sind, zwischen Raw, Reflectance und Radiance zu wechseln und die berechneten Daten direkt im Liniendiagramm darzustellen. Die kontinuierliche Aquisitation neuer Messwerte wird durch eine Endlosschlaufe immer weitergefŸhrt bis die Abbruchbedingung eintritt. Weiter ist die Klasse fŸr folgende Aktionen und Darstellungen zustŠndig:
...
Die Klassen Start, Settings und Finish werden hier nicht genauer erlŠutert, diese haben keinen grossen funktionellen Anteil und verwalten vor allem Einstellungen. Der genaue Ablauf in den MeasurementModals ist etwas komplizierter folgt aber immer der Implementierung in der Base-Klasse. Die Grundidee ist, ein Liniendiagramm darzustellen, das fortlaufend immer aktuelle Messwerte beim Spektrometer anfordert. Entscheidet sich der Benutzer nun Messungen durchzufŸhren werden die Messwerte zusŠtzlich Abgefangen und entsprechend Verarbeitet. FŸr die Darstellung werden die Messwerte entsprechend Berechnet in die effektiven Messdaten (Indico Dateien) werden aber nur die gemessenen Raw-Daten geschrieben. Die wichtigsten Komponenten der Klasse \verb=BaseMeasurementAquire= werden nachfolgend genauer beschrieben.

Die Funktion \verb=startAquire()= wird direkt nach der Instanzierung der jeweiligen Klasse in der Methode \verb=viewDidLoad()= aufgerufen. Mit diesem Aufruf beginnt das kontinuierliche Anfordern der Messdaten auf dem Spektrometer. Die Daten werden dann anhand der nachfolgenden beschriebenen Implementierung verarbeitet und angezeigt.

\begin{lstlisting}
func startMesurement()
\end{lstlisting}
Diese Methode Ÿbernimmt alle FunktionalitŠt sobald der Benutzer die Messwerte auch wirklich abspeichern mšchte. In dieser Methode werden dann auch die entsprechenden UI Komponenten initialisiert, aktiviert oder deaktiviert.

\begin{lstlisting}
func finishedMeasurement()
\end{lstlisting}
Sobald die Anzahl Messungen erreicht sind wird diese Methode aufgerufen. Das weitere Abspeichern und Abzweigen von Messdaten wird automatisch gestoppt. Diese Methode kann Ÿberschrieben werden sollten zum Messungsende weitere Schritte notwendig sein.
...
\end{lstlisting}
FŸr die Diagrammanzeige mŸssen jeweils noch Berechnungen am Spektrum durchgefŸhrt werden. Diese Methode \textbf{muss} ebenfalls zwingend Ÿberschrieben werden und die gewŸnschte FunktionalitŠt zu garantieren.
